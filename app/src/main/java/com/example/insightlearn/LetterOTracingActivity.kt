...
class LetterOTracingActivity : AppCompatActivity() {

    private lateinit var drawingView: ODotDrawingView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_letter_o_tracing)
        ...
        // Initial state
        instructionText.text = "Step 1: Trace the full circle of 'O'"
        ...
        drawingView.stepCompletedListener = { isFinished ->
            runOnUiThread {
                if (isFinished) {
                    instructionText.text = "🎉 Great! Letter 'O' completed!"
        ...
        nextButton.setOnClickListener {
            if (drawingView.isCompleted()) {
                val intent = Intent(this, LetterPTracingActivity::class.java)
                startActivity(intent)
                finish()
            } else {
                successMessage.text = "Please complete the letter!"
                successMessage.setTextColor(Color.RED)
                successMessage.visibility = View.VISIBLE
            }
        }

        // Optional: forward touch events to drawing view
