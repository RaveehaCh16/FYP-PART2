...
class RDotDrawingView @JvmOverloads constructor(
    ...
    private fun setupSteps(width: Float, height: Float) {
        ...
        // Step 2: Top curve (like half circle on top)
        val step2 = Path().apply {
            val rect = RectF(startX, startY, startX + 2 * radiusX, middleY)
            arcTo(rect, 270f, 180f)
        }

        // Step 3: Diagonal leg (bottom slant)
        val step3 = Path().apply {
            moveTo(startX + radiusX, middleY)
            lineTo(startX + 2 * radiusX, endY)
        }
