...
class LDotDrawingView @JvmOverloads constructor(
    ...
    private fun setupSteps(width: Float, height: Float) {
        ...
        // Step 1: Vertical line of 'L'
        val step1 = Path().apply {
            moveTo(startX, startY)
            lineTo(startX, endY)
        }

        // Step 2: Horizontal line of 'L' (extended)
        val step2 = Path().apply {
            moveTo(startX, endY)
            lineTo(width * 2 / 3, endY) // Extended endpoint for longer bottom line
        }
