...
class UDotDrawingView @JvmOverloads constructor(
    ...
    private fun setupSteps(width: Float, height: Float) {
        ...
        val radius = (endX - startX) / 2
        val arcRect = RectF(startX, bottomY - 2 * radius, endX, bottomY)

        // Full capital U path in one step
        val fullUPath = Path().apply {
            moveTo(startX, topY)
            lineTo(startX, bottomY - radius)
