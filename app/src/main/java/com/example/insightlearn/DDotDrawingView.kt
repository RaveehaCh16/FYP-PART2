...
class DDotDrawingView @JvmOverloads constructor(
    ...
    private fun setupSteps(width: Float, height: Float) {
        ...
        val radiusX = width / 6
        val radiusY = (endY - startY) / 2

        // Step 1: Vertical line (|) - the left stem of 'D'
        val step1 = Path().apply {
            moveTo(startX, startY)
            lineTo(startX, endY)
        }

        // Step 2: Curved arc for the round part of 'D'
        val step2 = Path().apply {
            val rect = RectF(startX, startY, startX + 2 * radiusX, endY)
            arcTo(rect, 270f, 180f)  // Top to bottom curve on the right
        }

        steps.addAll(listOf(step1, step2))
        setupStepPath()
    }
