...
class IDotDrawingView @JvmOverloads constructor(
    ...
    private fun setupSteps(width: Float, height: Float) {
        ...
        // Step 1: Top horizontal line
        val topLine = Path().apply {
            moveTo(startX, startY)
            lineTo(endX, startY)
        }

        // Step 2: Middle vertical line
        val verticalLine = Path().apply {
            moveTo(centerX, startY)
            lineTo(centerX, endY)
        }

        // Step 3: Bottom horizontal line
        ...
        steps.add(topLine)
        steps.add(verticalLine)
        steps.add(bottomLine)
