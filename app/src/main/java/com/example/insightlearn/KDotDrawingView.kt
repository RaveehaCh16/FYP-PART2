...
class KDotDrawingView @JvmOverloads constructor(
    ...
    private fun setupSteps(width: Float, height: Float) {
        ...
        val verticalX = width / 4
        val topY = height / 6
        val bottomY = height * 5 / 6
        val middleY = (topY + bottomY) / 2
        val diagonalX = width * 3 / 4  // Diagonal ends on the right

        // Step 1: Vertical Line
        val step1 = Path().apply {
            moveTo(verticalX, topY)
            lineTo(verticalX, bottomY)
        }

        // Step 2: Top Diagonal Line (middle to top-right)
        val step2 = Path().apply {
            moveTo(verticalX, middleY)
            lineTo(diagonalX, topY)
        }

        // Step 3: Bottom Diagonal Line (middle to bottom-right)
        val step3 = Path().apply {
            moveTo(verticalX, middleY)
            lineTo(diagonalX, bottomY)
        }
